  ...
  if (!$username || !$password) {
    ...
    if (!$user || $user['password'] !== $password) {
      ...
      if (in_array($normalized_role, $no_sport_roles)) {
        ...
        elseif (in_array($normalized_role, ['trainee', 'trainor'])) {
          // Get sport from training attendance or schedule
          $sport_stmt = $pdo->prepare("
            SELECT DISTINCT st.sports_id
        ...
        elseif (in_array($normalized_role, ['umpire', 'scorer'])) {
          // Get sport from match assignments or recent activity
          $sport_stmt = $pdo->prepare("
            SELECT sports_id 
            FROM tbl_match 
            WHERE umpire_id = :pid OR scorer_id = :pid
